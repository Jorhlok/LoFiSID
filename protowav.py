...
while not (quit and bufindex == 0 and extrabuf == 0):
    ...
    for x in range(clkperup):
        ...
        if clks >= sampskip:
            clks -= sampskip
            buf[bufindex] = int(max([min([samp/sampdiv*sampmax,sampmax]), -1*sampmax])) #average over several samples else thin pulses can get distorted
            samp = 0
            sampdiv = 0
            bufindex += 1
            if bufindex >= buflen:
                wavout.writeframes(buf)
                bufindex = 0
                if quit:
